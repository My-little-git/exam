...
@section('content')
    ...
    <section id="create-product">
        <div class="container-xxl">
            <h2 class="mt-5 mb-3">Редактирование товара</h2>
            <form action="{{ route('admin.product.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <input
                        name="year"
                        type="number"
                        value="{{ old('year') ?: $product->year ?: 2016}}"
                        min="1940"
                        max="{{ date("Y") }}"
                    ...
                    <select
                        ...
                        @foreach($models as $model)
                            ...
                            <option value="{{ $model->id }}"
                                {{ (old('model_id') ?: $product->model_id) === $model->id ? "selected" : "" }}>{{ $model->name }}</option>
                    ...
                    <select
                        ...
                        @foreach($countries as $country)
                            ...
                            <option value="{{ $country->id }}"
                                {{ (old('country_id') ?: $product->country_id) === $country->id ? "selected" : "" }}>{{ $country->name }}</option>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="description" class="form-control" id="description" style="height: 100px">{{ old('description') ?: $product->description }}</textarea>
                    @error('description')
                    <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
